...
- **PR 流程**：main 分支禁直推（分支保护已启用），所有改动走 feature 分支 → PR → 合入；单人阶段 PR 免审批直接 merge，多协作者后再收紧审批。
- **与 AscendNPU-IR 主仓的关系**：纯下游使用者（Python bindings 环境依赖，见 V1 记录）。发现的主仓 bindings 注册缺口及其修复方案已存档于 V1 执行记录与项目记忆备查；不主动上游化，未来需要时可按主仓 AGENTS.md 规范重做补丁。
