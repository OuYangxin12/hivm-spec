...
| 任务 | 内容 | 验收 | 状态 |
|---|---|---|---|
| T4.0 | z3 依赖决策（可选 extra vs 硬依赖；缺失时的降级口径） | 决策入档，缺 z3 报缺口而非静默跳过 | ✅ PR #25 |
| T4.1 | z3 后端接入符号句柄（受限子集提升为 SMT） | op 函数零改动切换符号模式 | ✅ PR #26 |
| T4.2 | 有界符号等价：值语义子集（限子集为设计约束，D4/OD6） | 子集内两 IR 等价出 SAT/UNSAT | ✅ PR #27 |
| T4.3 | 未初始化读检查：tainted 标记位 + 读判定 | 注入用例检出 | ✅ PR #24 |
| T4.4 | 同步静态配对检查：set/wait 配对完整性（账本级，快速） | 注入用例检出 | ✅ PR #23 |
| T4.5 | 性能复核 + D10 符号档预算行回填 | 实测数据入表 | ✅ PR #28 |

> **T4.3 的实现偏离已记录**：计划原写"具体模式 poison 填充"，实际用**独立
> tainted 标记位**。理由：魔数可能是合法计算结果（`0xDEADBEEF` 作 f32 是个正常
> 负数），NaN 更不安全——真实计算本就会产生 NaN，届时无法区分"读了未初始化
> 内存"与"算出了 NaN"。污染状态必须与数值分离。详见 PR #24 与 M4 卡 §4.5。
...
- **本地合入：仅限离线，且必须补 PR（M2 审查发现 2 → PR #11 实证后收紧）**。

  **远端规则是硬的**：直推 `main` 会被拒绝（`GH013: Changes must be made through a
  pull request`）。发现 2 起初被表述为"远端分支保护与 CI 结构性失效"，PR #11 的实证
  纠正了这个判断——保护规则一直有效，问题在于本地 merge 让它**根本没有机会执行**。
  绕过 CI 的不是规则漏洞，而是"改动从未接触远端"这个事实。

  因此口径为：

  1. **默认路径恒为 feature 分支 → PR → CI 全绿 → 合入**，不存在"本地合入"这一常规选项；
  2. 仅当**确实离线**时，允许本地 merge 到 main 作为临时手段，且必须先跑本地门禁：
  ...
  3. **恢复联网后必须立即补 PR**：把本地合入的提交推成分支并开 PR，交由远端 CI 裁决。
     在 CI 通过前，那些提交只能算"本地自测通过"，**不得据此宣称已满足 D5**。

  `merge_gate.py` 跑 CI 门禁的本地等价物（spec-gate 的 base 取 merge-base，避免漏检
  未推送提交），结果落成**可审计证据**（HEAD、时间、逐门禁结论、工作区是否干净）；
  工作区不干净即拒绝——否则"绿"不对应任何一个提交。它是**离线期间的过渡凭据**，
  **不是** CI 的替代品：CI 恒为唯一权威。

  实证补充（PR #11）：本地六门禁全绿的一批提交，远端 CI 仍额外覆盖了本地未跑的
  **py3.12** 一档。可见本地门禁与 CI 并不等价，"本地全绿"不足以推断"CI 会绿"。
- **机器门禁不可绕过（D5）**：单人阶段免人工审批，但 CI 门禁必须全绿方可合入——这是 FR6 防自欺的实际载体，不得以"单人阶段"为由跳过或放松。放松门禁的改动本身受 CODEOWNERS 看护。**离线期间的本地合入同受此条约束**（落点 `merge_gate.py`），但该路径只是过渡：补 PR 并让 CI 通过后，D5 才真正满足。（口径提示：CI 是 **4 个 job**，其中 `core` 经 py3.10/py3.12 矩阵展开，PR 上呈现为 **5 个 check**。）
