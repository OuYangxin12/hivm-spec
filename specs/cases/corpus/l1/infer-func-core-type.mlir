...
module {
  ...
  // A vector-only body would infer AIV; the ratio forces MIX.
  // CHECK-LABEL: func.func @vector_body_with_core_ratio(
  // CHECK-SAME: hivm.core_ratio = #hivm.core_ratio<0, 1>
  // CHECK-SAME: hivm.func_core_type = #hivm.func_core_type<MIX>
  func.func @vector_body_with_core_ratio(%base : memref<?xf32>,
                                         %indices : tensor<8xi64>,
                                         %dst : tensor<8xf32>) -> tensor<8xf32>
