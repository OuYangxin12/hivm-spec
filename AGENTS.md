...
## 3.5 核心命题：扩语义靠写描述，不靠改引擎（**读之前先明确**）

> 本项目**不是**"HIVM 语义验证工具"，而是**验证器生成器**。
> 核心命题：**MLIR 千变万化，正确性类别有限。**

这条决定了你面对"工具支持的 op 不够多"时该做什么。**默认动作是写描述，不是改
引擎。**
...
└─ 不能 → 需要新原语，走下面的"扩原语"流程       ← 少数，且须谨慎
```

**实测证据**（M3 末）：往描述里加一个此前完全不存在的 `hivm.hir.vdiv`，仅
`value="elementwise(div, a, b, into=out)"` 一行，`git status src/` 全空——
占用/时序/等价三个工具同时获得对它的支持，等价验证能报出
`首个发散：hivm.hir.vdiv@i0 @ …:12`。核心引擎里硬编码的 op 名是 **0** 个。

### 不要用"建模了几个 op / 方言共几个 op"衡量能力

这个比值**没有意义**，且会把人引向"引擎应内置所有 op 语义"——那正是本项目
开篇否定的路线。方言里几百个 op 落到值语义上只有十几种原语：一个
`elementwise` 就覆盖了全部 vadd/vsub/vmul/vdiv/vexp/…。

**正确的能力指标是原语覆盖率**（`VALUE_PRIMITIVES` 有几个、能表达哪些类别），
以及**在此原语集下描述已覆盖多少目标语料**。汇报能力时用后者，不要用前者。

### 扩原语的流程（少数情况）

`VALUE_PRIMITIVES` 是**封闭集合**——描述里写引擎不认识的原语会直接报错，不会
静默放行。理由：允许描述凭空声明语义，等于允许臆造语义（FR6）。

新增原语必须**同时**满足：

1. 在 `values.py` 提供**双模**实现（具体 + 符号），不可只做具体档；
2. 加进 `VALUE_PRIMITIVES` 并补解析器分支；
3. 说明它是一个**语义类别**而非某个 op 的特例——若只为一个 op 服务，那多半
   该走逃生舱（`host_fn`）而不是新原语。

**反例**：不要为了让 `hivm.hir.foo` 跑通就加一个 `foo_semantics` 原语。原语的
名字应当是 `broadcast`/`reduce`/`cast` 这种跨 op 复用的语义类别。

### 什么时候用逃生舱

语义确实无法声明式表达时（如 `mmadL1` 依赖分形布局代数 `OpLayoutInterface`），
用 `host_fn` 逃生舱并**写明理由**。工具会如实把它及其下游报为 `COVERAGE_GAP`
——这是正确行为，不是缺陷。D9 规定：未经对拍的语义不得臆造，猜一个布局代数
写进去才是真风险。
...
| **引擎不得硬编码 HIVM op 名**：新增 op 的默认路径是写描述；扩引擎仅限新增**跨 op 复用的语义原语**，且须双模实现 | §3.5 核心命题 | 退化成"逐个建造验证工具"，验证器生成器命题作废 |
